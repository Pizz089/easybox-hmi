...
**Deroga documentata — touch 44 sidebar.** Le voci di menu della SideBar e il collapse toggle usano il minimo touch **44px** (non il 52 preferito): il menu completo conta 17 voci e a 52px sforerebbe il viewport del pannello; il toggle è una pillola flottante 44×44 a metà schermo. Stessa classe di deroga della toolbar diag qui sotto.

**Regola sidebar — fit operatore (U-FASE2, vincolante per ogni voce futura).** Il set visibile all'**OPERATORE (userLevel 0) DEVE stare in 1080p senza scroll** (budget: 1016px utili sotto la TopBar). Per i livelli tecnici (voci `requiresLevel`: Macchine, MQTT Live) lo scroll della `.sb-nav` è accettato: sotto piega finiscono solo voci tecniche, mai operative. Le voci NON scendono sotto 44px (già in deroga): il fit si recupera dagli header di sezione, dimagriti a ~30px/blocco (padding verticale `--space-1`, margin 0, `--line-height-tight`). Costi unitari per il calcolo a ogni voce nuova: voce ≈ 46px, header sezione ≈ 30px, gap fra sezioni 16px, padding-top nav 16px. A oggi (2 gruppi MAGAZZINO/ATTREZZAGGIO al posto di CONFIGURAZIONE, inclusa la voce Attrezzaggi): operatore ≈ 884px ✅ (margine ~132px), tecnico ≈ 1024px (scroll ~8px, solo voci tecniche sotto piega).
