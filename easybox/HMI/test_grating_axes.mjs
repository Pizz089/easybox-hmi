...
console.log('\n1) TRAY_9 — griglia di riferimento (pezzo 40x70, SAFE 20/10, bordi 20); origine = angolo cassetto (origin-fix 14/9: tasca 1 = (h1, w1) = (65, 50))');
const vm9 = makeVm({ pieceX: 40000, pieceY: 70000, safex: 20, safey: 10 });
const p9 = pts(vm9);
check(p9.length === 91, '91 tasche (' + p9.length + ')');
check(eq(p9[0], 65000, 50000), 'SUB_POS 1  = (65000, 50000) = (margine lato corto, margine lato lungo)');
check(eq(p9[1], 65000, 110000), 'SUB_POS 2  = (65000, 110000)');
check(eq(p9[12], 65000, 770000), 'SUB_POS 13 = (65000, 770000)');
check(eq(p9[13], 145000, 50000), 'SUB_POS 14 = (145000, 50000)');
check(eq(p9[90], 545000, 770000), 'SUB_POS 91 = (545000, 770000)');
...
for (let i = 0; i < 91; i++) {
	...
	if (!eq(p9[i], 65000 + 80000 * col, 50000 + 60000 * row)) { gridOk = false; break; }
}
check(gridOk, 'TUTTE le 91 tasche: X = 65000 + 80000*col, Y = 50000 + 60000*row (col = SUB_POS-1 div 13)');
const xs9 = distinct(p9.map(p => p.X)), ys9 = distinct(p9.map(p => p.Y));
check(xs9.length === 7 && xs9[0] === 65000 && xs9[6] === 545000, '7 valori X da 65000 a 545000 (passo 80000 = pezzo.Y + SAFEY), margini 65 simmetrici su 610');
check(ys9.length === 13 && ys9[0] === 50000 && ys9[12] === 770000, '13 valori Y da 50000 a 770000 (passo 60000 = pezzo.X + SAFEX), margini 50 simmetrici su 820');
...
check(xs8.length === 4 && xs8.join() === '95000,235000,375000,515000', '4 colonne X: 95000, 235000, 375000, 515000 (passo 140000 = 120 + 20; margine 95 = 20 + 15 + 60)');
check(ys8.length === 13 && ys8[0] === 50000 && ys8[12] === 770000, '13 valori Y da 50000 a 770000 (passo 60000 = 40 + 20)');
check(eq(p8[0], 95000, 50000) && eq(p8[1], 95000, 110000) && eq(p8[12], 95000, 770000) && eq(p8[13], 235000, 50000) && eq(p8[51], 515000, 770000), 'SUB_POS 1 (95000,50000), 2 (95000,110000), 13 (95000,770000), 14 (235000,50000), 52 (515000,770000)');
