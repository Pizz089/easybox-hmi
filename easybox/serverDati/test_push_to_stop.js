...
//     attenzione a corsa e spessore della ganascia, che il form mandava da
//     sempre e il backend perdeva in silenzio.
...
// caso cella: deposito -18298, pezzo 1033 (Y 100600), pinza 26 (spessore 5000),
// ganascia morsa 150000 -> corsa 24700 (gli stessi numeri della vista)
const cella = { enabled: true, hasVice: true, yPlace: -18298, pieceY: 100600, viceClawLength: 150000, gripperThickness: 5000 };
const r1 = srv.pushQuotes(cella);
check(r1.status === 'OK', 'dati completi -> OK');
check(r1.yPush === -18298 - 50300 - 2500, 'spinta = deposito - pezzo/2 - MEZZO spessore ganascia (' + r1.yPush + ')');
check(r1.clearance === 24700, 'corsa = (ganascia - pezzo)/2 = 24700');
check(r1.yStop === r1.yPush + 24700, 'arrivo = spinta + corsa (' + r1.yStop + ')');
const casi = [
	...
	{ name: 'spessore ganascia mancante', v: Object.assign({}, cella, { gripperThickness: null }), st: 'NO_DATA' },
	...
	{ name: 'valori dispari (troncamento come SQL)', v: Object.assign({}, cella, { pieceY: 100601, viceClawLength: 150001, gripperThickness: 5001 }), st: 'OK' },
];
for (const c of casi) {
...
check(srv.pushQuotes(casi[0].v).yPush === null && srv.pushQuotes(casi[6].v).yStop === null, 'quote NULL quando l\'esito non e\' OK (come la vista)');
...
const ORD = { pieceID: '1033', gripperID: '26', viceID: '0', fixtureID: '1', palletID: '9', machineID: '1', quantity: '3', PP: '12' };
let r = call('GET /insertOrder', ORD, [{ recordset: [{ ris: 'OK' }] }]);
let t = r.q[0];
check(/DECLARE @push int = ISNULL\(\(SELECT CASE WHEN PUSH_TO_STOP = 1 THEN 2 ELSE 0 END FROM PIECE WHERE ID=1033\), 0\)/.test(t), 'il bit lo legge il SQL dall\'anagrafica pezzo: istantanea non falsificabile dal client');
check(/DECLARE @claw int = \(SELECT TOP 1 CLAW_LENGTH_Y FROM VICE WHERE PALLET_ID=9\)/.test(t), 'ganascia morsa dal pallet dell\'ordine');
check(/DECLARE @tick int = \(SELECT TOP 1 Tickness_CLAW FROM GRIPPER WHERE ID=26\)/.test(t), 'spessore ganascia dalla pinza dell\'ordine');
check(/ELSE IF @push <> 0 AND \(ISNULL\(@claw,0\) <= 0 OR ISNULL\(@tick,0\) <= 0 OR ISNULL\(@pieceY,0\) <= 0\) SELECT 'KO_PUSH_NO_DATA'/.test(t), 'guardia dati mancanti');
...
r = call('GET /updatePiece', { ID: '1033', FAMILY: 'F', DESCR: 'D', PARTPROGRAM: '12', MC1_ONLY: '0', MC2_ONLY: '0', MC3_ONLY: '0', PRISMA: '1', X: '1', Y: '1', Z: '1', Z_PICK: '1', Z_PLACE: '1', PUSH_TO_STOP: '1' }, [{}]);
...
r = call('GET /updateVice', { ID: '1', FAMILY: 'M', DESCR: 'D', STATUS: '2', X: '1', Y: '1', Z: '1', Z_CLAW: '1', Z_SINK_CLAW: '1', MAG: '1', MAG_POS: '1', POS_PLANT: '1', CLAW_LENGTH_Y: '150000' }, [{}]);
check(/CLAW_LENGTH_Y=150000/.test(r.q[0]), 'morsa: ganascia in update');
r = call('GET /updateVice', { ID: '1', FAMILY: 'M', DESCR: 'D', STATUS: '2', X: '1', Y: '1', Z: '1', Z_CLAW: '1', Z_SINK_CLAW: '1', MAG: '1', MAG_POS: '1', POS_PLANT: '1', CLAW_LENGTH_Y: '' }, [{}]);
check(/CLAW_LENGTH_Y=NULL/.test(r.q[0]), 'morsa: campo vuoto -> NULL (non misurata)');
r = call('GET /updateGripper', { ID: '26', FAMILY: 'P', DESCR: 'D', X_BODY: '1', Y_BODY: '1', Z_BODY: '1', X_CLAW: '1', Y_CLAW: '1', Z_CLAW: '1', STATUS: '2', POS_MAG: '2', POS_PLANT: '0', STROKE_CLAW: '10000', TICKNESS_CLAW: '5000' }, [{ rowsAffected: [1] }]);
...
check(/, Stroke_CLAW, Tickness_CLAW\)/.test(r.q[0]) && /10000,\s*10000`?/.test(r.q[0]), 'pinza: in INSERT..SELECT il fallback e\' il default di schema, non il nome colonna');
...
check(/p\.Y - pz\.Y\/2 - g\.Tickness_CLAW\/2/.test(view), 'quota di spinta identica alla formula del modulo condiviso');
check(/\(v\.CLAW_LENGTH_Y - pz\.Y\)\/2/.test(view), 'corsa identica alla formula del modulo condiviso');
